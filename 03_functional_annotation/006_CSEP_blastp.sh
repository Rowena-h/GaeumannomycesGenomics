...
seqtk subseq ${proteins} ${strain}_CSEPs.txt > ${out_dir}/${strain}_CSEPs.faa

source blast-2.10

blastp \
	-query ${out_dir}/${strain}_CSEPs.faa \
	-db phi-base_current.fas \
	-outfmt "6 qseqid sseqid evalue bitscore pident length" \
	-evalue 1e-25 \
	-out ${out_dir}/${strain}_phibase_blast.tsv \
	-num_threads ${SLURM_CPUS_PER_TASK}

#Filter for top bitscore result per gene
sort -r -n -k4 < ${out_dir}/${strain}_phibase_blast.tsv | awk '!x[$1]++' ${out_dir}/${strain}_phibase_blast.tsv | sort -k 1b,1  > ${out_dir}/${strain}_phibase_blast_tophits.tsv
#Add data to CSEPs file
join -a1 -a2 -t $'\t' -o 1.1 2.2 2.3 2.4 2.5 -1 1 -2 1 ${strain}_CSEPs.txt ${out_dir}/${strain}_phibase_blast_tophits.tsv > ${strain}_CSEPs.tsv
